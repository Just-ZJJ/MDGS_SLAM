...
def rot_compare(prev_rot: torch.Tensor, curr_rot: torch.Tensor):
    # 使用矩阵乘法 @ 替换 np.dot 或 np.matmul
    rot_diff = prev_rot.T @ curr_rot

    # PyTorch没有直接的 trace 方法，但是可以通过对角线元素求和来实现
    cos_theta = (torch.sum(torch.diagonal(rot_diff)) - 1) / 2

    rad_diff = torch.acos(cos_theta)
    theta_diff = torch.rad2deg(rad_diff)
    return rad_diff, theta_diff

def trans_compare(prev_trans: torch.Tensor, curr_trans: torch.Tensor):
    trans_diff = prev_trans - curr_trans
    l1_diff = torch.norm(trans_diff, p=1)  # L1 norm
    l2_diff = torch.norm(trans_diff, p=2)  # L2 norm
