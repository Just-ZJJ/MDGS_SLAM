...
def remove_outliers(vertex0, vertex1, mask, threshold=0.1):

    diff = torch.norm(vertex0 - vertex1, dim=-1)
    new_mask = mask & (diff < threshold)
    return new_mask
...
class ICP(nn.Module):
    ...
    def icp(self, pose10,vertex_t0,vertex_t1,normal_t0,normal_t1, K):
        ...
        # remove outliers
        mask_clean = remove_outliers(vertex_t0, vertex_t1, mask0)

        for idx in range(self.max_iterations):
            # compute residuals
            residuals, J_F_p, valid_mask = self.compute_residuals_jacobian(
                vertex_t0,vertex_t1,normal_t0,normal_t1, mask_clean, pose10, K,
...
class IcpTracker:
    def __init__(self, config):
        self.icp_trackers = []
        self.icp_downscales = config["icp_params"]["icp_downscales"]
        self.icp_warmup_frames = config["icp_params"]["icp_warmup_frames"]
        self.icp_use_model_depth = config["icp_params"]["icp_use_model_depth"]
        for iters in config["icp_params"]["icp_downscale_iters"]:
            self.icp_trackers.append(
                ICP(
                    iters,
                    distance_threshold=config["icp_params"]["icp_distance_threshold"],
                    normal_threshold=config["icp_params"]["icp_normal_threshold"],
                    damping=config["icp_params"]["icp_damping"],
                    verbose=config["icp_params"]["verbose"],
                )
            )

        self.depth_pyramid_builder = ImagePyramids(list(range(len(self.icp_downscales)-1,-1, -1)), "max")
        self.icp_sample_distance_threshold = config["icp_params"]["icp_sample_distance_threshold"]
        self.icp_sample_normal_threshold = config["icp_params"]["icp_sample_normal_threshold"]
        self.icp_fail_threshold = config["icp_params"]["icp_fail_threshold"]

        self.normal_pyramid_t0 = None
        self.vertex_pyramid_t0 = None
        self.verbose = config["icp_params"]["verbose"]
    ...
    def update_last_status(
        self,  render_depth, frame_depth, render_normal, frame_normal
    ):
        vertex_map_c = compute_vertex_map(render_depth, K)
    ...
    """
    def update_last_status(
            self, render_depth, frame_depth, opacity, K
    ):
        opacity_mask = (opacity > 0.95).squeeze(-1)
        frame_vertex = compute_vertex_map(frame_depth, K)
        frame_normal = compute_normal_map(frame_vertex)
        render_vertex=compute_vertex_map(render_depth, K)
        render_normal=compute_normal_map(render_vertex)
        ...
        depth_filling_mask =( (
                              ...
                              | (render_depth == 0)[..., 0]) & (frame_depth > 0.01)[..., 0] &opacity_mask

        #depth_filling_mask= (frame_depth > 0.01)[..., 0]
        render_depth[depth_filling_mask] = frame_depth[depth_filling_mask]
        self.last_model_depth = render_depth
