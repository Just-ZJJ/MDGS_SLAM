...
class GaussianModel:
    ...
    def map_densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
    ...
    def map_densify_and_clone(self, grads, grad_threshold, scene_extent, box3ds):
    ...
    def map_densify_and_prune(self, max_grad, min_opacity, extent, max_screen_size, cams, boxes):
        grads = self.xyz_gradient_accum / self.denom
        grads[grads.isnan()] = 0.0
        ...
        if cams is not None:
            for i, cam_0 in enumerate(cams):
                for j, cam_1 in enumerate(cams[i+1:]):
                    ...
                    box3ds.append(box3d)
        self.map_densify_and_clone(grads, max_grad, extent, box3ds)
        self.map_densify_and_split(grads, max_grad, extent)
